...
ROOT = Path().absolute()
DATA = ROOT / "code_organization" / "functions_vs_classes" / "data"
...
def cumulative_count(
    ...
    print(f"--- Analysing frequencues of {column_name} --- ")
    counter = Counter()
    for line in file:
        if field_sep:
...
def show_frequencies(counter: Counter[str]) -> None:
    for possibility, freq in counter.most_common():
        print(f"{possibility: <36} -> {freq: <5} | " f"{round(freq / counter.total() * 100, 2)}%")
    print("\n")


def show_all_answers(counter: Counter[str]) -> None:
    print("The possible answers found with the dataset are:")
    print(f"{';'.join(list(counter))}\n")


def analyze_frequencies(column_name: str, file: DictReader, field_sep: str | None = None) -> None:
...
def main() -> None:
    with open(DATA / FILENAME, "r", encoding="utf-8") as csv_file:
        ...
        csv_file.seek(1)
        analyze_frequencies(column_name="LanguageHaveWorkedWith", file=csv_reader, field_sep=";")
