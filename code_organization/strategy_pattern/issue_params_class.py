...
class StrategyParameters:
    """Class represents the union of possible parameters for the different strategies."""
    window_size: int = 3
...
class TradingBot:
    ...
    def __init__(self, exchange: Exchange, trading_strategy: TradingStrategy, strategy_params: StrategyParameters) -> None:
    ...
    def run(self, symbol: str) -> None:
        ...
        should_buy = self.trading_strategy.should_buy(prices, self.strategy_params)
        should_sell = self.trading_strategy.should_sell(prices, self.strategy_params)
        if should_buy:
...
def main():
    ...
    strategy_params = StrategyParameters(min_price=31000, max_price=34000)

    # create the trading bot and run the bot once
    bot = TradingBot(exchange, trading_strategy, strategy_params)
    bot.run("BTC/USD")
